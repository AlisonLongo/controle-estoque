...
$msg_bloqueio = $bloqueado ? "⚠️ Existe um inventário em andamento. Não é possível registrar novas movimentações." : "";
...
$origem_texto = ['AV'=>'Movimentação Avulsa', 'OC'=>'Ordem de Compra', 'IN'=>'Inventário'];
?>
...
<div class="container mt-4">
   ...
   <form method="POST" class="mb-4">
    ...
    <div class="mb-3">
        <label>Item:</label>
        <select name="id_item" id="item-select" class="form-select" required <?= $bloqueado ? '' : '' ?>>
    ...
    <div class="mb-3">
        ...
        <input type="number" name="valor_unitario" id="valor-unitario" class="form-control" step="0.01" value="0" required <?= $bloqueado ? '' : '' ?>>
    </div>

    <div class="mb-3">
        <label>Tipo:</label>
        <select name="tipo_movimentacao" class="form-select" required <?= $bloqueado ? '' : '' ?>>
    ...
    <div class="mb-3">
        ...
        <input type="number" name="quantidade" class="form-control" value="" min="1" required <?= $bloqueado ? '' : '' ?>>
    </div>

   <button type="submit" name="salvar" class="btn btn-primary" <?= $bloqueado ? 'disabled' : '' ?>>
    <?= $bloqueado ? 'Registro bloqueado' : 'Registrar' ?>
</button>
    ...
    <table class="table table-striped">
        ...
        <?php foreach($movs as $m): ?>
            <tr>
                ...
                <td><?= $origem_texto[$m['observacao']] ?? $m['observacao'] ?></td> <!-- Exibe origem traduzida -->
            </tr>
        <?php endforeach; ?>
        </tbody>
    </table>
